...
class UploadForm(forms.Form):
  ...
  ## filename = forms.FileField(widget=forms.FileInput(attrs={'class':'form-control','accept':'.mp3','id':'file'}))
   filename = RestrictedFileField(accept="audio/wav",max_upload_size=1048576) 


class SearchForm(forms.Form):
   search = forms.CharField(widget=forms.TextInput(attrs={'class':'form-control','placeholder':'Enter Name or Tags ','style':'padding:10px;width:500px;'}))
